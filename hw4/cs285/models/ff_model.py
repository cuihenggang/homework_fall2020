...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        next_obs_pred, delta_pred_normalized = self.forward(obs, acs, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)
        prediction = ptu.to_numpy(next_obs_pred)  # TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        obs_delta = next_observations - observations  # TODO(Q1) compute the normalized target for the model.
        target = normalize(obs_delta, data_statistics["delta_mean"], data_statistics["delta_std"])
        ...
        delta_mean = ptu.from_numpy(data_statistics["delta_mean"])
        delta_std = ptu.from_numpy(data_statistics["delta_std"])
        next_obs_pred, delta_pred_normalized = self.forward(observations, actions, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)
